...
class OAuthTokensApiTest(TestCase):

    def test_override_api_context(self):
        with self.settings(OAUTH_TOKENS_API_CALL_CONTEXT={}):
            self.assertEqual(settings.OAUTH_TOKENS_API_CALL_CONTEXT, {})
            with override_api_context('vkontakte', user=1):
                self.assertEqual(settings.OAUTH_TOKENS_API_CALL_CONTEXT, {'vkontakte': {'user': 1}})
                with override_api_context('vkontakte', token='abc'):
                    self.assertEqual(settings.OAUTH_TOKENS_API_CALL_CONTEXT, {'vkontakte': {'user': 1, 'token': 'abc'}})
